...
import com.taskflow.task_scheduler_service.infrastructure.exceptions.ResourceNotFoundException;
...
public class TasksService {
    ...
    public List<TaskDTO> findByEventDateBetween
        ...
        return taskConverter.convertToDTOList(
                tasksRepository.findByEventDateBetweenAndNotificationStatusEnum(startDate, endDate, NotificationStatusEnum.PENDING));
    }
    ...
    public void deleteTaskById(String taskId) {
        try {
            ...
            throw new ResourceNotFoundException("Task with ID '" + taskId + "' not found. Deletion failed.", e.getCause());
        }
    ...
    public TaskDTO updateStatusNotification(NotificationStatusEnum status, String taskId) {
      try {
          ...
          throw new ResourceNotFoundException("Task with ID '" + taskId + "' not found. Status update failed.", e.getCause());
      }
    }
    ...
    public TaskDTO updateTask(TaskDTO dto, String taskId) {
        try {
            ...
            throw new ResourceNotFoundException("Task with ID '" + taskId + "' not found. Update failed.", e.getCause());
        }
    }
